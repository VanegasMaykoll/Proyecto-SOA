...
const FilterPolicy* NewXorFilterPolicy(double bloom_equivalent_bits_per_key, int /*bloom_before_level*/) {
  class XorFilterPolicy : public BloomLikeFilterPolicy {
   public:
    explicit XorFilterPolicy(double bits_per_key) : BloomLikeFilterPolicy(bits_per_key) {}

    FilterBitsBuilder* GetBuilderWithContext(const FilterBuildingContext&) const override {
      return new XorFilterBitsBuilder(static_cast<int>(GetMillibitsPerKey() / 1000.0 + 0.5));
    }

    const char* CompatibilityName() const override { return "xorfilter"; }
    const char* Name() const override { return "rocksdb.XorFilter"; }
  };
  return new XorFilterPolicy(bloom_equivalent_bits_per_key);
}
