...
// Filter precision parameter.
// For Bloom, this is the physical number of bits per key.
// For Ribbon and Xor, the custom policy must interpret this value as the
// precision target documented by that implementation (preferably
// Bloom-equivalent bits per key for a controlled comparison).
// A negative value disables filtering.
static int FLAGS_bloom_bits = 10;

// Select one custom filter at a time. If both are false, Bloom is used.
...
static const FilterPolicy* CreateFilterPolicy() {
  if (FLAGS_bloom_bits < 0) {
    return NULL;
  }

  if (FLAGS_use_ribbon_filter) {
    return NewRibbonFilterPolicy(FLAGS_bloom_bits);
  }

  if (FLAGS_use_xor_filter) {
    return NewXorFilterPolicy(FLAGS_bloom_bits);
  }

  return NewBloomFilterPolicy(FLAGS_bloom_bits);
}
...
class Benchmark {
 private:
  ...
  void PrintHeader() {
    ...
    if (filter_policy_ == NULL) {
      fprintf(stdout, "Filter:     none\n");
    } else {
      ...
      if (FLAGS_use_ribbon_filter) {
        fprintf(stdout,
                "FilterMode: Ribbon\n"
                "FilterBits: %d Bloom-equivalent bits/key\n",
                FLAGS_bloom_bits);
      } else if (FLAGS_use_xor_filter) {
        fprintf(stdout,
                "FilterMode: Xor\n"
                "FilterBits: %d precision target\n",
                FLAGS_bloom_bits);
      } else {
        fprintf(stdout,
                "FilterMode: Bloom\n"
                "FilterBits: %d physical bits/key\n",
                FLAGS_bloom_bits);
      }
    }
...
int main(int argc, char** argv) {
  ...
  if (FLAGS_bloom_bits == 0 || FLAGS_bloom_bits < -1) {
    fprintf(stderr,
            "Invalid --bloom_bits value %d. Use -1 to disable filters or "
            "a positive number.\n",
            FLAGS_bloom_bits);
    return 1;
  }
  ...
  if (FLAGS_use_ribbon_filter && FLAGS_use_xor_filter) {
    fprintf(stderr,
            "Ribbon and Xor cannot be enabled simultaneously. Choose only "
  ...
  if ((FLAGS_use_ribbon_filter || FLAGS_use_xor_filter) &&
      FLAGS_bloom_bits <= 0) {
    fprintf(stderr,
            "Ribbon and Xor require --bloom_bits to be positive.\n");
    return 1;
  }
